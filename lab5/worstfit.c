...
int main()
{
    ...
    for (int i = 0; i < numProcesses; i++)
    {
        ...
        if (worstIdx != -1)
        {
            allocation[worstIdx] = i;                 // Allocate process to this partition
            partitions[worstIdx] -= processes[i];     // Reduce available partition size
            wastage[worstIdx] = partitions[worstIdx]; // Calculate wastage
        }
    }
      // display results
    for (int i = 0; i < numPartitions; i++)
